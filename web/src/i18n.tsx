...
export const zh = {
	...
	themeDark: "深色",
	newChat: "新对话",
	newChatEphemeral: "临时对话",
	newChatEphemeralTip: "临时对话（仅内存，关闭即销毁，不进历史记录）",
	ephemeralBadge: "临时",
	ephemeralBannerText: "当前为临时对话：消息仅保存在内存中，关闭后自动销毁，不进入历史记录。",
	more: "更多",
	brand: "品牌",
	...
	hostProcessor: "CPU",
	hostMemory: "RAM",
	hostResourcesTip: "运行 pi-web-ui 服务的主机 · CPU 为约 10 秒平均（2 秒采样），与任务管理器采样时刻不同会有几秒偏差",
	...
	forkSession: "派生分支",
	forkSessionTip: "从此消息处派生新分支会话（截取该位置之前的转录）",
	rollbackSession: "回滚到此",
	rollbackSessionTip: "丢弃此消息之后的内容，在当前会话直接继续",
	rollbackConfirm: "确定要回滚到此消息吗？该消息之后的所有对话将被丢弃，并在当前会话继续。",
	...
	toolApprovalCommand: "命令内容",
	toolApprovalParams: "工具调用参数",
	toolApprovalEditPlaceholder: "就地修改参数 JSON…",
	toolApprovalReason: "拦截原因",
	toolApprovalCategory: "规则档位",
	toolApprovalAllowCategory: "本对话允许同类",
	toolApprovalAllowCategoryHint: "本对话内这一类高危操作后续不再询问（设置 →「工具」页可撤销）",
	toolApprovalAllowConversation: "本对话全部允许",
	toolApprovalAllowConversationHint: "本对话内所有高危操作后续都不再询问（设置 →「工具」页可撤销）",
	toolApprovalEnabled: "工具执行审批（人机协同）",
	toolApprovalEnabledDesc:
		"开：高危操作（rm -rf / 破坏性 git / 敏感文件 / 工作区外写入 / 插件要求确认）弹窗等你批准、拒绝或改参数；关：一切审批都不弹（内置高危检测直接放行，插件要求确认也按放行处理）。",
	toolApprovalPolicyTitle: "已记住的放行（本对话）",
	toolApprovalPolicyAllowAll: "本对话全部允许审批",
	toolApprovalPolicyRevoke: "撤销",
	toolApprovalPolicyHint: "只存在内存里：重启服务或新对话即恢复询问；策略跟对话走（手动过户时一起搬）。",
	settingsApprovalRules: "审批规则",
	settingsApprovalRulesDesc:
		"全局自定义人机协同拦截规则，支持基于正则/通配符/路径对工具命令或参数配置「需审批」、「直接拒绝」或「免审放行」。规则自顶向下首条匹配优先。",
	manageApprovalRules: "管理审批规则",
	approvalRuleNew: "新增规则",
	approvalRuleEdit: "编辑规则",
	approvalRuleDelete: "删除",
	approvalRuleReset: "恢复默认",
	approvalRuleResetConfirm: "确认将该内置规则恢复为系统默认设定吗？",
	approvalRuleDeleteConfirm: "确认删除该自定义审批规则吗？",
	approvalRuleActionAsk: "需审批",
	approvalRuleActionDeny: "直接拒绝",
	approvalRuleActionAllow: "免审放行",
	approvalRuleTools: "适用工具",
	approvalRuleToolsTip: "输入工具名（如 bash、write、edit），以逗号分隔；输入 * 表示匹配所有工具",
	approvalRuleField: "检查字段",
	approvalRuleFieldCommand: "命令 (command)",
	approvalRuleFieldPath: "文件路径 (path)",
	approvalRuleFieldParams: "完整参数 JSON (params)",
	approvalRuleMatch: "匹配方式",
	approvalRuleMatchRegex: "正则表达式 (regex)",
	approvalRuleMatchGlob: "通配符 (glob)",
	approvalRuleMatchContains: "包含子串 (contains)",
	approvalRuleMatchPrefix: "前缀开头 (prefix)",
	approvalRuleMatchOutsideWs: "工作区外路径 (outside_workspace)",
	...
	noSkills: "暂无技能",
	noExtensions: "暂无插件",
	noPresets: "暂无预设（调整好当前设置后，在上方输入名称保存为预设）",
	...
	planUpdateEnabledDesc:
		"结构化任务计划更新（Plan Mode / Step State Machine）：让模型将复杂任务拆解为步骤并实时更新推进状态。",
	planUpdateOffHint: "已关闭：AI 不能再更新结构化计划看板",
	compactContextEnabledDesc:
		"根据当前问题主动压缩上下文：允许 AI 自主指定压缩重点、保留与当前任务相关的核心上下文并去除无关历史细节，自主控制保留范围并在回合后生效。",
	compactContextOffHint: "已关闭：AI 不能主动调用上下文压缩工具（仍可用 /compact 命令或触发软上限自动压缩）",
	evalEnabledDesc:
	...
	lspToolEnabledDesc:
		"原生语言服务器协议（LSP）工具：为 AI 提供 IDE 级的代码语义智能（支持定义跳转、引用查询、类型悬停、编译诊断、符号大纲 documentSymbol、精确符号代码切片 read_symbol、工作区全局符号搜索 workspaceSymbol），多子代理共享进程池与闲置休眠，免外部插件开箱即用。",
...
const en: Record<keyof typeof zh, string> = {
	...
	themeDark: "Dark",
	newChat: "New chat",
	newChatEphemeral: "Ephemeral chat",
	newChatEphemeralTip: "Ephemeral chat (in-memory only, auto-discarded on close, not saved to history)",
	ephemeralBadge: "Ephemeral",
	ephemeralBannerText: "Ephemeral session: messages are in-memory only, discarded on close, not saved to history.",
	more: "More",
	brand: "Brand",
	...
	hostMemory: "RAM",
	hostResourcesTip:
		"Host running pi-web-ui · CPU is a ~10s average (2s samples); a few seconds of skew vs Task Manager is normal",
	...
	saveEphemeral: "Save as regular conversation",
	...
	forkSessionTip: "Fork a new branch session from this message (truncates prior transcript)",
	rollbackSession: "Rollback to here",
	rollbackSessionTip: "Discard all content after this message and continue here",
	rollbackConfirm: "Are you sure you want to rollback to this message? All subsequent messages will be discarded.",
	rollbackRestoreWorkspace: "Restore workspace files to this checkpoint (Dual-State Rollback)",
	...
	toolApprovalParams: "Tool Parameters",
	toolApprovalEditPlaceholder: "Modify parameter JSON in place…",
	toolApprovalReason: "Reason for Interception",
	toolApprovalCategory: "Category",
	toolApprovalAllowCategory: "Allow this kind here",
	toolApprovalAllowCategoryHint:
		"Stop asking for this category of high-risk operations in this conversation (revocable under Settings → Tools)",
	toolApprovalAllowConversation: "Allow all here",
	toolApprovalAllowConversationHint:
		"Stop asking for any high-risk operation in this conversation (revocable under Settings → Tools)",
	toolApprovalEnabled: "Tool execution approval (human-in-the-loop)",
	toolApprovalEnabledDesc:
		"On: high-risk operations (rm -rf / destructive git / sensitive files / writes outside the workspace / plugin confirmation requests) pop a dialog for you to approve, deny or edit the parameters. Off: no approvals at all (built-in detection passes silently, plugin confirmation requests are treated as allowed).",
	toolApprovalPolicyTitle: "Remembered allowances (this conversation)",
	toolApprovalPolicyAllowAll: "All approvals allowed in this conversation",
	toolApprovalPolicyRevoke: "Revoke",
	toolApprovalPolicyHint:
		"In-memory only: restarting the service or opening a new conversation restores prompting. The policy travels with the conversation (e.g. on manual takeover).",
	settingsApprovalRules: "Approval Rules",
	settingsApprovalRulesDesc:
		"Global human-in-the-loop approval rules. Configure ask, deny, or allow actions for tool calls based on regex/glob/path matching. Evaluated top-to-bottom on first match.",
	manageApprovalRules: "Manage Approval Rules",
	approvalRuleNew: "Add Rule",
	approvalRuleEdit: "Edit Rule",
	approvalRuleDelete: "Delete",
	approvalRuleReset: "Reset to Default",
	approvalRuleResetConfirm: "Reset this built-in rule to system defaults?",
	approvalRuleDeleteConfirm: "Are you sure you want to delete this custom approval rule?",
	approvalRuleActionAsk: "Ask",
	approvalRuleActionDeny: "Deny",
	approvalRuleActionAllow: "Allow",
	approvalRuleTools: "Target Tools",
	approvalRuleToolsTip: "Comma-separated tool names (e.g. bash, write, edit); use * to match all tools",
	approvalRuleField: "Target Field",
	approvalRuleFieldCommand: "Command (command)",
	approvalRuleFieldPath: "File Path (path)",
	approvalRuleFieldParams: "Full Params JSON (params)",
	approvalRuleMatch: "Match Mode",
	approvalRuleMatchRegex: "Regular Expression (regex)",
	approvalRuleMatchGlob: "Glob Pattern (glob)",
	approvalRuleMatchContains: "Contains Substring (contains)",
	approvalRuleMatchPrefix: "Prefix (prefix)",
	...
	noPresets: "No presets yet (tune your current settings, then enter a name above to save them as a preset)",
	...
	compactContextOffHint:
		"Disabled: AI cannot proactively invoke context compaction (you can still use /compact or trigger auto-compaction)",
	evalEnabledDesc:
	...
	lspToolEnabledDesc:
		"Native Language Server Protocol (LSP) tool: IDE-grade semantic code intelligence (definition, references, hover, diagnostics, documentSymbol outline, read_symbol precise slicing, and workspaceSymbol search). Multi-agent shared process pool with idle sleep, zero external dependencies.",
